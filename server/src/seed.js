...
const gasto = (pid, insumo, qtd, valor, daysAgo, descricao) => {
  db.prepare("INSERT INTO gastos (id, user_id, plantio_id, insumo_id, descricao, quantidade, valor_unitario, data) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").run(
    uuid(), uid, pid, insumo.id, descricao, qtd, valor, addDays(-daysAgo)
  );
};

const colheita = (pid, qtd, preco, daysAgo, unidade = "kg") => {
  db.prepare("INSERT INTO colheitas (id, user_id, plantio_id, data, quantidade, unidade, preco_venda) VALUES (?, ?, ?, ?, ?, ?, ?)").run(
    uuid(), uid, pid, addDays(-daysAgo), qtd, unidade, preco
  );
};

lotes.forEach(seedLote);
insumos.forEach(seedInsumo);

// Alface — em andamento, colheita próxima
const p1 = plantio(lotes[1], "Alface", 35, 12, { cultivar: "Crespa", qtd: 300, unidade: "pés", status: "ativo" });
gasto(p1, insumos[0], 3, 18.5, 35, "Semente para 300 pés");
gasto(p1, insumos[1], 8, 6.4, 33, "Cobertura inicial");
gasto(p1, insumos[4], 2, 22, 10, "Preventivo pragas");

// Tomate — colhido, ciclo encerrado
const p2 = plantio(lotes[1], "Tomate", 110, 0, { cultivar: "Santa Cruz", qtd: 60, unidade: "mudas", status: "colhido", previsto: addDays(-35) });
gasto(p2, insumos[3], 60, 1.2, 110, "Mudas 60 un");
gasto(p2, insumos[1], 25, 6.4, 90, "Manutenção ciclo");
gasto(p2, insumos[4], 3, 22, 40, "Controle de pragas");
colheita(p2, 420, 4.5, 35);
colheita(p2, 260, 4.0, 18);

// Cenoura — planejada
const p3 = plantio(lotes[0], "Cenoura", 2, 95, { cultivar: "Nantes", qtd: 500, unidade: "mudas", status: "planejado" });
gasto(p3, insumos[1], 10, 6.4, 2, "Preparo do solo");

// Couve — em andamento, colheita próxima
const p4 = plantio(lotes[2], "Couve", 45, 7, { cultivar: "Manteiga", qtd: 150, unidade: "pés", status: "ativo" });
gasto(p4, insumos[2], 20, 3.8, 40, "Adubação orgânica");
